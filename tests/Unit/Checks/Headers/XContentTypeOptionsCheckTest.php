...
it('exposes correct metadata', function () {
    $check = new XContentTypeOptionsCheck($this->app);

    expect($check->id())->toBe('headers.x-content-type-options')
...
it('passes when matching middleware is registered', function () {
    ...
    $reflection->getProperty('middlewareGroups')->setValue($kernel, [
        'web' => ['App\\Http\\Middleware\\NoSniffHeaders'],
    ]);

    expect(iterator_to_array((new XContentTypeOptionsCheck($this->app))->run()))->toBeEmpty();
});
...
it('fails when no matching middleware is present', function () {
    ...
    $findings = iterator_to_array((new XContentTypeOptionsCheck($this->app))->run());
